...
window.rotateDailyAffirmation = () => {
    ...
    affirmationEl.textContent = `"${DAILY_AFFIRMATIONS[randomIndex]}"`;
};
...
window.addSupportJarMessage = () => {
    ...
    jar.push({ text, addedAt: new Date().toISOString() });
...
window.receiveSupportJarMessage = () => {
    const output = document.getElementById('support-jar-output');
    const jar = JSON.parse(localStorage.getItem('zepol_support_jar') || '[]');
    const defaults = [
    ...
    const message = jar.length ? jar[Math.floor(Math.random() * jar.length)].text : defaults[Math.floor(Math.random() * defaults.length)];
    ...
    if (validCodes.length === 0) {
        ...
        html += validCodes.map(c => `
            <div class="dls-player-card" style="background: white; border: 1px solid #e2e8f0; color: var(--text-primary); padding: 15px; border-radius: 12px; text-align: center; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
                ...
                <div style="font-size: 0.75rem; color: #a0aec0; margin-bottom: 10px;">Rete ${Math.ceil((TEN_MINUTES - (now - new Date(c.timestamp).getTime())) / 60000)} minit</div>
            </div>
        `).join('');
    }
